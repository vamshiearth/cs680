...
class DirectoryTest {
    ...
    private String[] dirToStringArray(Directory d) {
        return new String[] {
                d.getName(),
                String.valueOf(d.getTotalSize()),    // FIXED
                d.getCreationTime().toString()       // Keep this as-is
        };
    }


    @Test
    void testEqualityCheckImplementationDir() {
        FileSystem fileSystem = TestFixtureInitializer.getFileSystem();
        Directory umbcs680 = fileSystem.getRootDirs().get(0);
        Directory hw01 = umbcs680.getSubDirectories().get(0);
        Directory src = hw01.getSubDirectories().get(0);

        String[] expectedRoot = {
                umbcs680.getName(),
                String.valueOf(umbcs680.getTotalSize()),
                umbcs680.getCreationTime().toString()
        };
        assertArrayEquals(expectedRoot, dirToStringArray(umbcs680));

        String[] expectedHw01 = {
                hw01.getName(),
                String.valueOf(hw01.getTotalSize()),
                hw01.getCreationTime().toString()
        };
        assertArrayEquals(expectedHw01, dirToStringArray(hw01));

        String[] expectedSrc = {
                src.getName(),
                String.valueOf(src.getTotalSize()),
                src.getCreationTime().toString()
        };
        assertArrayEquals(expectedSrc, dirToStringArray(src));
    }
